...
import static com.leo.common.widget.easyswipemenulayout.State.CLOSE;

/**
 * 
 * @author wcs
 * 
 * @Package cn.com.drpeng.libzeus.widget.easyswipemenulayout
 * 
 * @Description item横向滑动控件（gradle无法下载，则复制源码至lib，地址：https://github.com/anzaizai/EasySwipeMenuLayout）
 * 
...
public class EasySwipeMenuLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //1、获取配置的属性值
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.common_EasySwipeMenuLayout, defStyleAttr, 0);

        try {
            int indexCount = typedArray.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int attr = typedArray.getIndex(i);
                if (attr == R.styleable.common_EasySwipeMenuLayout_common_leftMenuView) {
                    mLeftViewResID = typedArray.getResourceId(R.styleable.common_EasySwipeMenuLayout_common_leftMenuView, -1);
                } else if (attr == R.styleable.common_EasySwipeMenuLayout_common_rightMenuView) {
                    mRightViewResID = typedArray.getResourceId(R.styleable.common_EasySwipeMenuLayout_common_rightMenuView, -1);
                } else if (attr == R.styleable.common_EasySwipeMenuLayout_common_contentView) {
                    mContentViewResID = typedArray.getResourceId(R.styleable.common_EasySwipeMenuLayout_common_contentView, -1);
                } else if (attr == R.styleable.common_EasySwipeMenuLayout_common_canLeftSwipe) {
                    mCanLeftSwipe = typedArray.getBoolean(R.styleable.common_EasySwipeMenuLayout_common_canLeftSwipe, true);
                } else if (attr == R.styleable.common_EasySwipeMenuLayout_common_canRightSwipe) {
                    mCanRightSwipe = typedArray.getBoolean(R.styleable.common_EasySwipeMenuLayout_common_canRightSwipe, true);
                } else if (attr == R.styleable.common_EasySwipeMenuLayout_common_fraction) {
                    mFraction = typedArray.getFloat(R.styleable.common_EasySwipeMenuLayout_common_fraction, 0.5f);
                }
            }
